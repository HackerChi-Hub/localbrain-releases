...
- **Mac：1.3.12**，适用于 Apple Silicon。网页检查没能返回结果时不再报成「页面可能死循环」，交付也不会被这样一次中断永久拦住。设置合并到一个入口、按九类分栏，每项只显示关键词，长说明收进悬浮提示；对话页「参数」与设置页用同一套界面。读取目录与可写工作区分开授权；项目构建／测试命令须逐次确认；网页预览可选择受限联网。
