...
- **Mac：1.3.18**，适用于 Apple Silicon。修正模型卡片标题：此前标题会钉死某一个量化档（如「· Q4_K_M」），而卡片实际按本机内存自动选更高的档，内存大的机器上标题和真正下载的档位对不上、体积差一倍。13 条受影响的条目一起改；各档本来就同一量化的（如官方 QAT、Bonsai）保留原标题。
